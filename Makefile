...
C_FILES           := $(filter-out %.inc.c,$(foreach dir,$(SRC_DIRS),$(wildcard $(dir)/*.c))) $(LEVEL_C_FILES)
...
# Build ROM
ifeq ($(VERSION),cn)
  PAD_TO_GAP_FILL := --pad-to=0x1000000 --gap-fill=0x00
else
  PAD_TO_GAP_FILL := --pad-to=0x1000000 --gap-fill=0xFF
endif
