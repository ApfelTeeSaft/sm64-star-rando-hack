...
static void play_current_area_music(void) {
...
void init_mario_after_warp(void) {
    ...
    if (randomizer_should_block_level_start()) {
        randomizer_begin_blocking_level_start();
        if (!gWarpTransition.isActive && !gWarpTransition.pauseRendering) {
            play_transition(WARP_TRANSITION_FADE_INTO_COLOR, 1, 0, 0, 0);
        }
        level_set_transition(2, randomizer_transition_update);
...
void initiate_warp_force_level(s16 destLevel, s16 destArea, s16 destWarpNode, s32 arg3) {
    ...
    if (sDelayedWarpOp == WARP_OP_NONE) {
        ...
        switch (warpOp) {
            ...
            case WARP_OP_STAR_EXIT:
               if (randomizer_is_active()) {
                    sDelayedWarpTimer = 8;
                    sSourceWarpNodeId = WARP_NODE_SUCCESS;
                    gSavedCourseNum = COURSE_NONE;
                    val04 = FALSE;
                    play_transition(WARP_TRANSITION_FADE_INTO_COLOR, 0x08, 0x00, 0x00, 0x00);
                } else {
    ...
    if (sDelayedWarpOp != WARP_OP_NONE && --sDelayedWarpTimer == 0) {
        ...
        if (gDebugLevelSelect && (sDelayedWarpOp & WARP_OP_TRIGGERS_LEVEL_SELECT)) {
            ...
            switch (sDelayedWarpOp) {
                ...
                case WARP_OP_DEATH:
                case WARP_OP_WARP_FLOOR:
                    if (randomizer_handle_quick_retry()) {
                        sDelayedWarpOp = WARP_OP_NONE;
                        break;
                    }
                ...
                case WARP_OP_STAR_EXIT:
                    if (randomizer_handle_star_exit_warp()) {
                        sDelayedWarpOp = WARP_OP_NONE;
                        break;
                    }
...
s32 play_mode_normal(void) {
    ...
    if (gPlayer1Controller->buttonPressed & U_JPAD) {
        randomizer_handle_quick_retry();
    }

    if (sTimerRunning && gHudDisplay.timer < 17999) {
        gHudDisplay.timer++;
    }
    randomizer_on_level_update();
...
#if ENABLE_RANDOMIZER_VOICE_PLAYBACK
        if (randomizer_finish_blocking_level_start()) {
            play_transition(WARP_TRANSITION_FADE_FROM_COLOR, 8, 0, 0, 0);
            play_current_area_music();
            set_play_mode(PLAY_MODE_NORMAL);
            return 0;
        }
...
s32 lvl_init_from_save_file(UNUSED s16 arg0, s32 levelNum) {
    ...
    levelNum = randomizer_override_initial_level(levelNum);
    gCurrLevelNum = levelNum;
...
s32 lvl_set_current_level(UNUSED s16 arg0, s32 levelNum) {
    ...
    if (randomizer_should_skip_act_selector()) {
        return 0;
    }
