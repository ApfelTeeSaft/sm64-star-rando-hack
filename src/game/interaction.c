...
u32 interact_coin(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (COURSE_IS_MAIN_COURSE(gCurrCourseNum)
        ...
        && (!randomizer_is_active() || randomizer_current_star_is_100_coin())) {
...
u32 interact_star_or_key(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (m->health >= 0x100) {
        ...
        if (!randomizer_current_star_matches(gCurrCourseNum, gCurrLevelNum, starIndex)) {
            o->oInteractStatus = 0;
            return FALSE;
        }
        save_file_collect_star_or_key(m->numCoins, starIndex);
        randomizer_on_star_collected(gCurrCourseNum, gCurrLevelNum, starIndex);
        if (randomizer_should_force_star_exit()) {
            noExit = FALSE;
