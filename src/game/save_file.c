...
void save_file_collect_star_or_key(s16 coinScore, s16 starIndex) {
    ...
    if (courseIndex >= COURSE_NUM_TO_INDEX(COURSE_MIN)
        ...
        && gSaveBuffer.files[fileIndex][0].capLevel != RANDO_SAVE_MAGIC
#endif
    ) {
...
#if ENABLE_RANDOMIZER
    if (gSaveBuffer.files[gCurrSaveFileNum - 1][0].capLevel == RANDO_SAVE_MAGIC) {
        flags &= ~RANDO_LOST_CAP_FLAGS;
    }
#endif
...
#if ENABLE_RANDOMIZER
        if (gSaveBuffer.files[fileIndex][0].capLevel == RANDO_SAVE_MAGIC) {
            switch (courseIndex + 1) {
                case COURSE_BITDW:
                case COURSE_BITFS:
                case COURSE_BITS:
                case COURSE_COTMC:
...
#if ENABLE_RANDOMIZER
    if (saveFile->capLevel == RANDO_SAVE_MAGIC) {
        return;
    }
#endif
