...
#if ENABLE_RANDOMIZER_VOICE_PLAYBACK
static u8 get_bank_from_sequence_set(s32 seqId, s32 bankIndex) {
    u16 offset = ((u16 *) gAlBankSets)[seqId];
    u8 count = gAlBankSets[offset];

    if (count == 0) {
        return 0;
    }

    if (bankIndex < 0 || bankIndex >= count) {
        bankIndex = 0;
    }

    return gAlBankSets[offset + count - bankIndex];
}
#endif
...
#if ENABLE_RANDOMIZER_VOICE_PLAYBACK
    if (seqId == SEQ_RANDOMIZER_VOICE) {
        bankId = get_bank_from_sequence_set(seqId, gRandomizerVoiceBankIndex);
        if (IS_BANK_LOAD_COMPLETE(bankId) == TRUE) {
