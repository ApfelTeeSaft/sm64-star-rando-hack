...
#include "game/randomizer_menu.h"
...
#endif
static u8 textRandomizer[] = {
    ASCII_TO_DIALOG('R'), ASCII_TO_DIALOG('A'), ASCII_TO_DIALOG('N'),
    ASCII_TO_DIALOG('D'), ASCII_TO_DIALOG('O'), DIALOG_CHAR_TERMINATOR
};
...
void render_score_menu_buttons(struct Object *scoreButton) {
    ...
    s32 fileIndex = sSelectedFileNum > 0 ? sSelectedFileNum - 1 : 0;

    randomizer_menu_open(fileIndex);
    return;
#endif
    // File A
...
void check_score_menu_clicked_buttons(struct Object *scoreButton) {
    ...
    if (scoreButton->oMenuButtonState == MENU_BUTTON_STATE_FULLSCREEN) {
        s32 startFile = randomizer_menu_consume_start_file();

        if (startFile >= 0) {
            sSelectedFileNum = startFile + 1;
            return;
        }

        if (randomizer_menu_should_close()) {
            play_sound(SOUND_MENU_CAMERA_ZOOM_OUT, gGlobalSoundSource);
#if ENABLE_RUMBLE
            queue_rumble_data(5, 80);
#endif
            randomizer_menu_clear_close();
            sCurrentMenuLevel = MENU_LAYER_MAIN;
            scoreButton->oMenuButtonState = MENU_BUTTON_STATE_SHRINKING;
        }
    }

    if (scoreButton->oMenuButtonState == MENU_BUTTON_STATE_DEFAULT) {
...
#if ENABLE_RANDOMIZER
        randomizer_select_file(fileNum - 1);
#endif
...
void handle_cursor_button_input(void) {
    ...
    if (sSelectedButtonID == MENU_BUTTON_SCORE) {
        if (sMainMenuButtons[MENU_BUTTON_SCORE]->oMenuButtonState == MENU_BUTTON_STATE_FULLSCREEN) {
            randomizer_menu_handle_input();
        }
        return;
    }
#endif
...
void print_main_menu_strings(void) {
    ...
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
#if ENABLE_RANDOMIZER
    print_generic_string(SCORE_X, 39, textRandomizer);
#else
...
void print_main_lang_strings(void) {
    ...
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, sTextBaseAlpha);
#if ENABLE_RANDOMIZER
    centeredX = get_str_x_pos_from_center(76, textRandomizer, 10.0f);
    print_generic_string(centeredX, 39, textRandomizer);
#else
...
#if ENABLE_RANDOMIZER
    randomizer_menu_render(sTextBaseAlpha);
    return;
#endif
#ifdef VERSION_EU
    s16 centeredX;
